...
class Tray:
    ...
    def __init__(self, app, icon_path: str) -> None:
        ...
        menu.addAction("Afficher", self.window.show_and_activate)
        menu.addAction("Patterns plein écran", self.window.show_patterns)
        menu.addAction("Calibrage guidé", self.window.show_calibration_wizard)
        menu.addSeparator()

        self.act_focus = QAction("Mode Focus", menu)
        ...
        menu.addAction("Sauver l'instantané",    self.window.save_snapshot)
        menu.addAction("Restaurer l'instantané", self.window.restore_snapshot)
        menu.addSeparator()
